...
template <class T>
concept RequiresRawAccess = std::is_void_v<T> || std::is_empty_v<T>;


template<  // c++20 version
        std::integral T,        // the raw type must be an integer
...
public:
    ...
    // if the discriminator is void or an empty class provide a raw_value function. c++20 version
    auto raw_value() requires RequiresRawAccess<DISC> { return id; }
